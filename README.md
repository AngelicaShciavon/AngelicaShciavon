<div align="center">

# 👋 Olá! Eu sou a Angélica Barbosa

### 💙 Analista de Qualidade de Software (QA)

🎓 Estudante de Análise e Desenvolvimento de Sistemas

🔎 Focada em Qualidade de Software, Testes Manuais e Automação de Testes.

</div>

---

# 👩‍💻 Sobre mim

Sou estudante de **Análise e Desenvolvimento de Sistemas** e estou construindo minha carreira como **Analista de Qualidade de Software (QA)**.

Tenho experiência na elaboração e execução de testes, documentação de funcionalidades, identificação de falhas e validação de regras de negócio, buscando sempre garantir a qualidade das aplicações.

Atualmente estou aprofundando meus conhecimentos em **Automação de Testes**, **JavaScript**, **TypeScript**, **Banco de Dados** e **Boas Práticas de Engenharia de Software**.

---

# 🧪 Áreas de atuação

✔ Testes Manuais                    ✔ Testes Funcionais

✔ Testes Exploratórios              ✔ Azure DevOps

✔ Testes de Regressão               ✔ Evidências de Teste

✔ Testes de Interface (UI)          ✔ Planos de Teste

✔ Testes de Usabilidade             ✔ Cenários de Teste

✔ Validação de Regras de Negócio    ✔ Casos de Teste

✔ Registro de Bugs

---

# 🚀 Tecnologias

<p align="left">

<img src="https://skillicons.dev/icons?i=html" height="45"/>

<img src="https://skillicons.dev/icons?i=css" height="45"/>

<img src="https://skillicons.dev/icons?i=javascript" height="45"/>

<img src="https://skillicons.dev/icons?i=typescript" height="45"/>

<img src="https://skillicons.dev/icons?i=nodejs" height="45"/>

<img src="https://skillicons.dev/icons?i=mysql" height="45"/>

<img src="https://skillicons.dev/icons?i=git" height="45"/>

<img src="https://skillicons.dev/icons?i=github" height="45"/>

<img src="https://skillicons.dev/icons?i=vscode" height="45"/>

</p>

---

# 🛠 Ferramentas
- Azure DevOps             - Visual Studio Code         
- Azure Test Plans         - GitHub
- Cypress                  - Git
- Playwright               - MySQL Workbench
---

# 📚 Atualmente estudando
- Cypress           - GitHub Actions
- Playwright        - Banco de Dados Relacional
- JavaScript        - SQL
- TypeScript
---

# 📂 Projetos

## 🧪 Testes Manuais
- Casos de Teste
- Cenários de Teste
- Plano de Testes
- Relatórios de Bugs
- Evidências de Testes

---

## 🤖 Automação
- Cypress
- Playwright
- RobotFramework

---

## 💻 Desenvolvimento
- HTML
- CSS
- JavaScript
- TypeScript
- Node.js

---

## 🗄 Banco de Dados

| Categoria | Tecnologias |
|-----------|-------------|
| **Relacionais** | MySQL • PostgreSQL • SQL Server • Oracle Database • MariaDB • SQLite |
| **NoSQL** | MongoDB • Firebase Firestore • Redis |
| **Linguagem** | SQL (DDL, DML, DQL) |
| **Modelagem** | MER • Modelo Relacional • Normalização |
| **Consultas** | SELECT • JOIN • GROUP BY • ORDER BY • HAVING |
| **Objetos** | Views • Índices • Procedures • Triggers |
| **Ferramentas** | MySQL Workbench |
| **Conceitos** | Integridade Referencial • Chaves Primárias e Estrangeiras • Relacionamentos (1:1, 1:N e N:N) |

# 📊 Sequência de contribuições

![GitHub Streak](https://streak-stats.demolab.com?user=AngelicaShciavon&theme=tokyonight)

---

# 📫 Vamos nos conectar?

<a href="https://www.linkedin.com/in/ang%C3%A9lica-schiavon-barbosa-27755b106/">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="mailto:angelicaschiavonbarbosa@gmail.com">
<img src="https://img.shields.io/badge/E--mail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://github.com/AngelicaShciavon">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

---

<div align="center">

### 💙 "Qualidade não é apenas encontrar defeitos, mas contribuir para a construção de software confiável."

</div>
